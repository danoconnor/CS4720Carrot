...
public class MainActivity extends Activity {
	...
	private void initializeMainPage()
	{
		...
		rateButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				initializePickClassForRatingPage();
			}
		});
		...
		OnClickListener listener = new OnClickListener() {
			...
			public void onClick(View view) {
				...
				String reqName = rowStringParts[0]; // first word should be req name
		...
		try {
			JSONObject result = new WebServiceTask().execute("http://plato.cs.virginia.edu/~cs4720s14carrot/user/" + username).get();
			...
			if (result != null)
			{
				...
				header.setText("You have " + numClasses + " classes left to take!");
	...
	{
		final TextView header = (TextView)findViewById(R.id.header);
		header.setText("The following classes fill the " + requirement + " requirement. Choose one to see information about that class:");
		
		// call web service and get list of results that meet the requirement
		// for each result, add a row to the LinearLayout with the class name, button to add class, button to view class ratings
		
		try {
			// TODO update to the proper web service
			JSONObject result = new WebServiceTask().execute("http://plato.cs.virginia.edu/~cs4720s14carrot/getclasses/" + requirement).get();
			...
			if (result != null)
			{
				...
				ArrayAdapter<String> reqAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, data);
	...
	private void initializeViewClassesPage()
	{
		...
		// for each result, add a row to the LinearLayout with the class name and a button to add/update a class rating
		...
		try {
			JSONObject result = new WebServiceTask().execute("http://plato.cs.virginia.edu/~cs4720s14carrot/taken/" + username).get();
			
			if (result != null)
			{
				ArrayList<String> alreadyRated = new ArrayList<String>();
				JSONObject alreadyRatedResult = new WebServiceTask().execute("http://plato.cs.virginia.edu/~cs4720s14carrot/alreadyrated/" + username).get();
				
				if (alreadyRatedResult != null)
				{
					Iterator<String> alreadyIt = (Iterator<String>)(alreadyRatedResult.keys());
	...
	private void initializeAddClassForm(String dept, String c)
	{
		...
		addClassButton.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				String deparment = departmentField.getText().toString().toUpperCase();
				String courseNum = courseNumField.getText().toString();
				
				String url = "http://plato.cs.virginia.edu/~cs4720s14carrot/update/" + username;
				
				try {
					JSONObject result = new WebServiceTask().execute(url, "department", deparment, "courseNum", courseNum).get();
					...
					else
					{
						...
						Toast toast = Toast.makeText(v.getContext(), "Class not found, please try again", Toast.LENGTH_SHORT);
						toast.show();
					}
				} catch (Exception e)
				{
	...
	private void initializeRatingForm(String className)
	{
		...
		submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String url = "http://plato.cs.virginia.edu/~cs4720s14carrot/rate/" + username + "/" + dept + "/" + courseNum + "/" + prof + "/" + instValue + "/" + diffValue + "/" + timeValue + "/" + interValue;
				try {
	...
	{		
		...
		loginButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				{
					String checkUserUrl = "http://plato.cs.virginia.edu/~cs4720s14carrot/check/" + username;
					try {
						JSONObject result = new WebServiceTask().execute(checkUserUrl).get();
						if (result != null)
						{
							...
							else
							{
								...
								Toast toast = Toast.makeText(arg0.getContext(), "User '" + username + "' does not exist", Toast.LENGTH_SHORT);
								toast.show();
							}
						}
						else
						{
							Toast toast = Toast.makeText(arg0.getContext(), "Problem communicating with the login service\nPlease try again later", Toast.LENGTH_SHORT);
							toast.show();
						}
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
	...
	private void initializeViewRatingPage(String dept, String courseNum)
	{
		...
		try {
			// TODO update to the proper web service
			JSONObject result = new WebServiceTask().execute("https://cs4720sp14carrot.azurewebsites.net/" + dept + "/" + courseNum).get();
			...
			if (result != null)
			{
				...
				while (it.hasNext())
				{
					...
					RelativeLayout.LayoutParams classLayout = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
					...
					if (result.getString(profId).length() > 0)
					{
						...
						RelativeLayout.LayoutParams rowLayout = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
						TextView instrRating = new TextView(this);
						instrRating.setText("Instructor Rating: " + ratingsList.get(0));
						instrRating.setLayoutParams(rowLayout);
						linearLayout.addView(instrRating);
						
						TextView diffRating = new TextView(this);
						diffRating.setText("Difficulty Rating (1=easy, 5 = hard): " + ratingsList.get(1));
						diffRating.setLayoutParams(rowLayout);
						linearLayout.addView(diffRating);
						
						TextView timeRating = new TextView(this);
						timeRating.setText("Time Commitment (1= little time, 5=all your time): " + ratingsList.get(2));
						timeRating.setLayoutParams(rowLayout);
						linearLayout.addView(timeRating);
						
						TextView intRating = new TextView(this);
						intRating.setText("Difficulty Rating (1=easy, 5 = hard): " + ratingsList.get(3));
					...
					RelativeLayout.LayoutParams sepLayout = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
					sepLayout.height = 1;
	...
	private void initializeCreateUserPage()
	{
		...
		ArrayAdapter<CharSequence> gradAdapter = ArrayAdapter.createFromResource(this, R.array.grad_years, android.R.layout.simple_spinner_item);
		...
		submitButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				{
					String checkUserUrl = "http://plato.cs.virginia.edu/~cs4720s14carrot/check/" + newUsername;
					try {
						JSONObject result = new WebServiceTask().execute(checkUserUrl).get();
						String userExists = result.getString("userexists");
						...
						if (userExists.equals("false"))
						{
							if (pw1.getText().toString().equals(pw2.getText().toString()))
							{
								if (pw1.getText().length() >=  8)
								{
									...
									Random random = new Random(newUsername.hashCode());
									random.nextBytes(salt);
									KeySpec spec = new PBEKeySpec(pw1.getText().toString().toCharArray(), salt, 65536, 128);
									...
									JSONObject addUserResult = new WebServiceTask().execute(url, "username", newUsername, "password", hashedPassword, "firstname", fName.getText().toString(), "lastname", lName.getText().toString(), "major", (String)(majorSpinner.getSelectedItem()), "graduation", (String)(gradSpinner.getSelectedItem())).get();
									
									String success = addUserResult.getString("success");
									if (success.equals("true"))
									{
										Toast toast = Toast.makeText(v.getContext(), "User successfully created, please login", Toast.LENGTH_SHORT);
										toast.show();
									}
									else
									{
										...
										Toast toast = Toast.makeText(v.getContext(), "ERROR: " + success, Toast.LENGTH_SHORT);
										toast.show();
									}
								...
								else
								{
									...
									Toast toast = Toast.makeText(v.getContext(), "Password must be at least 8 characters", Toast.LENGTH_SHORT);
									toast.show();
								}
							}
							else
							{
								...
								Toast toast = Toast.makeText(v.getContext(), "Password Fields Do Not Match!", Toast.LENGTH_SHORT);
								toast.show();
							}
						}
						else
						{
							...
							Toast toast = Toast.makeText(v.getContext(), "User '" + newUsername + "' already exists", Toast.LENGTH_SHORT);
							toast.show();
						}
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
	...
	private void initializeNextSemesterPage()
	{
		...
		try {
			// TODO update to the proper web service
			JSONObject result = new WebServiceTask().execute("http://plato.cs.virginia.edu/~cs4720s14carrot/getclasses/").get();
			if (result != null)
			{
				Iterator<String> it = (Iterator<String>)(result.keys());
	...
	private void initializePickClassForRatingPage()
	{
		final Spinner deptSpinner = (Spinner) findViewById(R.id.deptSpinner);
		
		ArrayList<String> deptData = new ArrayList<String>();
		
		ArrayAdapter<String> deptAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, deptData);
		
		final Spinner courseNumSpinner = (Spinner) findViewById(R.id.courseNumSpinner);
		
		final Spinner profSpinner = (Spinner) findViewById(R.id.profSpinner);
